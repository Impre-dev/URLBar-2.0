...
(function () {
  ...
  const URLBar20 = {
    log(msg) {
      console.log('%c[URLBar-2.0]', 'color:#00d4ff;font-weight:bold', msg);
    },
    ...
    set floatingActive(v) {
      if (this._floatingActive && !v) {
        ...
        if (urlbar) {
          ...
          setTimeout(() => {
            urlbar.style.removeProperty('display');
          }, 50);
        }
      }
    ...
    async loadIcons() {
      ...
      try {
        ...
        /* favicon-map.json manquant, fallback générique */
      }
